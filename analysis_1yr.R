...
ggplot(data = lasso_cc_data) +
  ...
  scale_ggsurvfit(
    ...
    x_scales = list(breaks = seq(0, 365, by = 50), limits = c(0, 365))
  ) +
...
initial_model_metrics <- data.frame(
  ...
  surv_1yr = c(lasso_1yr_surv, ridge_1yr_surv, EL_1yr_surv, adapt_1yr_surv)
)
...
#function to calculate average 1-year predicted survival probability from the Cox model holding the donor factors fixed to a patients values while allowing recipient factors to vary.
standardized_risk_1yr <- function(data, model, donor_vars, time) {
  ...
  for (i in seq_len(nrow(data))) {
    ...
    # Survival probabilities at 1 year
    surv_prob <- S0^exp(lp)
...
std_risk_1yrs <- standardized_risk_1yr(
  data = data_train,
  model = cox_mle_model,
...
data_train_std <- data_train %>%
  mutate(std_risk_1yr = std_risk_1yrs)

eps <- 1e-6

#logit transform the marginalized donor coefficients
data_train_std <- data_train_std %>%
  mutate(logit_std_risk = qlogis(pmin(pmax(std_risk_1yr, eps), 1 - eps)))
