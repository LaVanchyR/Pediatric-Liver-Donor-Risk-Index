...
legend_df <- data.frame(
  ...
  surv = c("90.2%", "86.0%", "82.9%", "76.8%", "88.9%"),
  x = 1775,
...
dri_km_13_18 <- ggplot(data = dri_km_13_18_data) +
  ...
  scale_ggsurvfit(
    y_scales = list(breaks = seq(0.5, 1, by = 0.1), limits = c(0.5, 1)),
    x_scales = list(breaks = seq(0, 2750, by = 250), limits = c(0, 2750))
  ) +
  theme_ggsurvfit_default() +
  scale_fill_jama() +
...
risk_lookup_tbl <- risk_lookup_data %>%
  ...
  tab_footnote(
    footnote = c(
      "The 5-Year Graft Loss Survival Probability for Living Donors was 88.9%"
    ),
