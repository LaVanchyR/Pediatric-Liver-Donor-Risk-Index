...
#date: 8/13/2026
#purpose: Manage data for the pediatric donor risk index data.
...
#filter dataset to be between 6/30/2004 and 2026-03-01, and be a deceased donor recipient, have an age less than 18 years, and signal their initial transplant. 
liver_cohort <- liver_cohort %>%
  filter(REC_TX_DT >= "2004-06-30" & REC_TX_DT <= "2026-01-01" & DON_TY == "C" & REC_AGE_AT_TX < 18) %>%
  group_by(PERS_ID) %>%
  slice_min(REC_TX_DT) %>%
  ungroup()
...
liver_cohort_clean <- liver_cohort %>%
  mutate(death_date = case_when(!is.na(TFL_DEATH_DT) ~ TFL_DEATH_DT,
         ...
         one_year_censor = as_date(REC_TX_DT %m+% months(12)),
         last_fu_date = pmin(graft_failure_date, TFL_ENDTXFU, one_year_censor, na.rm = T),
         ...
         gs_outcome = case_when(last_fu_date == graft_failure_date ~ 1,
                                last_fu_date == TFL_ENDTXFU | last_fu_date == one_year_censor ~ 0,
                                TRUE ~ NA),
        ...
        tx_year = factor(substr(REC_TX_DT,1,4),
                         levels = c("2013","2004","2005","2006","2007","2008","2009","2010","2011","2012",
                                    ...
                                    "2024","2025")),
        donor_type = factor(case_when(REC_TX_PROCEDURE_TY %in% c(701,704) ~ "Whole",
                                      REC_TX_PROCEDURE_TY == 702 ~ "Partial",
                                      REC_TX_PROCEDURE_TY == 703 ~ "Split",
                                      TRUE ~ NA),
...
liver_cohort_clean <- liver_cohort_clean %>%
  select(-c(artificial_liver, donor_hist_mi, donor_intracranial_cancer, donor_dcd, donor_hist_cancer, donor_recov_out_us, tipss))
...
tbl_summary(
  data = liver_cohort_clean,
  include = c(age_at_transplant_cat, donor_age_at_tx_cat, blood_type, donor_blood_type, albumin, bmi_cat, cold_ischemic_time, donor_creat, creat, donor_ddavp, donor_death_circum, donor_death_mech, donor_hist_diab, diagnosis, dialysis_within_last_week, ebv_status, last_enceph, functional_status, sex, donor_sex, donor_hist_cig, donor_hist_hyperten, donor_inotrop, inr, life_support, prev_malig, med_condition, ventilator, pvt, prev_ab_surg, donor_sgot, bilirubin, donor_org_shared, exception, donor_cmv_status, prev_tx, donor_heavy_alc, donor_arginine, donor_blood_infect, donor_bun, donor_cdc_high_risk, donor_clinical_infect, donor_cod, donor_infect_other, donor_protein_urine, donor_diuretics, donor_steroids, donor_t3, donor_t4, donor_infect_lung, donor_bilirubin, donor_infect_urine, donor_vasodil, waitlist_time_cat, growth_failure, tx_year, donor_type, donor_bmi_cat),
...
liver_cohort_clean <- liver_cohort_clean %>%
  select(gs_time, gs_outcome, age_at_transplant_cat, age_at_transplant, donor_age_at_tx_cat, blood_type, donor_blood_type, albumin, bmi_cat, donor_bmi_cat, cold_ischemic_time, donor_creat, creat, donor_ddavp, donor_death_circum, donor_death_mech, donor_hist_diab, diagnosis, dialysis_within_last_week, ebv_status, last_enceph, functional_status, sex, donor_sex, donor_hist_hyperten, donor_inotrop, inr, life_support, prev_malig, med_condition, ventilator, pvt, prev_ab_surg, donor_sgot, bilirubin, donor_org_shared, exception, donor_cmv_status, prev_tx, donor_arginine, donor_blood_infect, donor_bun, donor_cdc_high_risk, donor_clinical_infect, donor_cod, donor_infect_other, donor_protein_urine, donor_diuretics, donor_steroids, donor_t3, donor_t4, donor_infect_lung, donor_bilirubin, donor_infect_urine, donor_vasodil, waitlist_time_cat, growth_failure, tx_year, donor_type)
...
analysis_1yr <- data_imputed_1


#write out the analysis dataset
save(analysis_1yr, file = "L:/Projects/Investigator/Jonathan Merola/data/analysis_1yr.RData")